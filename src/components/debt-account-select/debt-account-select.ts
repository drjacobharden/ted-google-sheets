...
const template = () => `
  <div class="form-field debt-account-select-field">
    <span class="debt-account-select-label">Debt account</span>
    <input class="id-input" type="hidden" />
    <dropdown-menu class="debt-account-menu" variant="editorial"
      label="Choose an account" icon="box" searchable search-action
      search-action-label="Add" search-placeholder="Search or add debt account" align-center>
    </dropdown-menu>
  </div>
`;
...
(function () {
  let nextId = 0;
  class DebtAccountSelect extends HTMLElement {
    ...
    #getOptions = () =>
      APIs.debt.accounts().filter((item) => item.active !== false);
    #createOption = (name) => APIs.debt.addAccount({ name });
    #onCreate = (account) => {
      this.dispatchEvent(
        new CustomEvent("debt-account-created", {
          bubbles: true,
          detail: { account },
        }),
      );
    ...
    setFallbackSelection(selection) {
      ...
      this.#fallbackSelection = id
        ? {
            id,
            name: String(selection?.name || "Archived debt account"),
            archived: true,
          }
        : null;
    ...
    connectedCallback() {
      ...
      const id = `debt-account-select-${++nextId}`;
      this.#dropdown.id = id;
      this.#dropdown.setAttribute("aria-label", "Choose a debt account");
      this.querySelector(".debt-account-select-label").id = `${id}-label`;
    ...
    handleEvent(event) {
      ...
      else if (event.type === "budget:debts-changed") this.#refresh(this.value);
    }
    ...
    #setValue(id, announce = false) {
      ...
      if (announce && item)
        this.dispatchEvent(
          new CustomEvent("debt-account-selected", {
            bubbles: true,
            detail: { account: item },
          }),
        );
    ...
    async #addAccount(input) {
      ...
      try {
        ...
        this.reportSelectionError(
          error?.message || "Unable to add debt account",
        );
      }
    }
  }
  customElements.define("debt-account-select", DebtAccountSelect);
})();
