...
import {
  ...
  type SpendTrendPeriod,
} from "../utilities/spend-trend";
...
function buildBudgetOverviewState(
  ...
  return {
    ...
    annualSpendTrendsByYear: Object.fromEntries(
      years.map((year) => [
        year,
        {
...
function updateBudgetOverviewState(): void {
  appState.set("budgetOverview", buildBudgetOverviewState(transactions));
}
...
function updateDerivedTransactionState(): void {
  ...
  const annualSummaryCards = buildAnnualSummaryCards(
    ...
    APIs.accounts
      ...
      .filter(
        (item) => item.activityType === "contribution",
      ) as import("../api/investment-types").InvestmentContribution[],
  );
  const years = [
    ...new Set([
      ...Object.keys(monthlyTransactionSummaries).map(Number),
...
export async function initializeData(
  options: { refresh?: boolean; startup?: boolean } = {},
): Promise<unknown> {
  if (options.refresh) {
    appDataPromise = null;
    loaded = false;
  ...
  if (!loaded && cached) {
    ...
    loaded = true;
    updateDerivedTransactionState();
    emit("budget:transactions-loaded", { source: "cache" });
  }
  const connected = Boolean(APIs.budget.getConfig().endpoint);
  emit("budget:data-refresh-started", {
    source: cached ? "cache" : "network",
    coldStart: Boolean(options.startup && connected && !loaded),
    connected,
  });
  appDataPromise = APIs.budget
    .loadAppData({ refresh: options.refresh })
    .then(async (data) => {
    ...
    .catch((error: unknown) => {
      ...
      emit("budget:data-refresh-failed", {
        error,
        showingCachedData: loaded,
        connected,
      });
      emit(
        "budget:api-warning",
        loaded
          ? "Showing saved data. Couldn’t refresh Google Sheets."
...
window.addEventListener("budget:transaction-queued", (event: Event) =>
  upsert([(event as CustomEvent).detail.transaction]),
);
window.addEventListener("budget:transactions-queued", (event: Event) =>
  upsert((event as CustomEvent).detail.transactions ?? []),
);
window.addEventListener("budget:transaction-saved", (event: Event) =>
  upsert((event as CustomEvent).detail.saved ?? []),
);
window.addEventListener("budget:transaction-restored", (event: Event) =>
  upsert([(event as CustomEvent).detail.transaction]),
);
window.addEventListener("budget:transaction-removed", (event: Event) => {
  transactions = transactions.filter(
    (item) => item.id !== (event as CustomEvent).detail.id,
  );
  updateDerivedTransactionState();
});
window.addEventListener("budget:transaction-sync-changed", (event: Event) => {
  ...
  transactions = transactions.filter(
    (item) => !item.syncStatus || ids.has(item.id),
  );
  upsert(queued);
});
  ...
  getBudgetOverviewAssignments: () => {
    ...
    APIs.budget.listAllPeople().forEach((assignment) => {
      if (!assignments.has(assignment.id))
        assignments.set(assignment.id, assignment.name);
    });
    return [...assignments]
      .map(([id, name]) => ({ id, name }))
      .sort((a, b) => a.name.localeCompare(b.name));
  },
  getTransaction: (id: string): BudgetTransaction | null =>
    transactions.find((item) => item.id === id) ?? null,
  areTransactionsLoaded: (): boolean => loaded,
  isReferenceDataLoaded: (): boolean => referenceDataLoaded,
  renameEntityTransactions(
    kind: "category" | "vendor" | "assignment",
    id: string,
    name: string,
  ): void {
    const idField = {
    ...
    }[kind] as keyof BudgetTransaction;
    const nameField = {
      category: "category",
      vendor: "vendor",
    ...
    transactions = transactions.map((item) =>
      item[idField] === id ? { ...item, [nameField]: name } : item,
    );
    updateDerivedTransactionState();
  },
};
