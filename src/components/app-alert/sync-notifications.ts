...
export class SyncNotifications extends HTMLElement implements EventListenerObject {
  ...
  handleEvent(event: Event): void {
    ...
    else if (event.type === "budget:sync-failed") this.#toasts?.show(`${count} ${count === 1 ? "change needs" : "changes need"} attention.`, { type: "error", sticky: true, action: "View Sync", onAction: () => router.navigate("settings", { section: "sync" }) });
    else if (!this.#outage?.isConnected) this.#outage = this.#toasts?.show("Couldn’t reach Google. Changes are saved locally and will retry automatically.", { type: "error", sticky: true, action: "View Sync", onAction: () => router.navigate("settings", { section: "sync" }) }) ?? null;
  }
}
