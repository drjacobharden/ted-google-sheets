...
export function AccountAPI(budget:BudgetAPIContract):AccountAPIContract {
  ...
  const getSyncItems=()=>outbox().map(item=>({key:item.id,source:item.kind==="month"?"accountMonth":item.kind==="balance-delete"?"accountBalance":"account",id:item.id,status:item.status,error:item.error,record:item.draft||item.record,current:item.current}));
