document.addEventListener("DOMContentLoaded", () => {
  ...
  function render() {
    ...
    const vendors = allVendors.filter((vendor) => vendor.name.toLowerCase().includes(query));
    document.getElementById("vendor-count").textContent = query ? `${vendors.length} of ${allVendors.length} vendors` : `${allVendors.length} ${allVendors.length === 1 ? "vendor" : "vendors"}`;
    if (!allVendors.length) {
      vendorList.innerHTML = '<div class="entity-empty"><strong>No vendors yet</strong><span>Add your first vendor to use it on transactions.</span></div>';
      return;
    }
    if (!vendors.length) {
      vendorList.innerHTML = '<div class="entity-empty"><strong>No matching vendors</strong><span>Try a different search.</span></div>';
      return;
    }
    vendorList.innerHTML = vendors.map((vendor) => {
      ...
      const syncControls = sync ? `<div class="entity-sync-state ${sync.status}"><span>${sync.status === "failed" ? "Needs attention" : "Pending"}</span>${sync.status === "failed" ? `<button type="button" data-entity-action="retry" data-entity-id="${vendor.id}">Retry</button><button type="button" data-entity-action="remove" data-entity-id="${vendor.id}">Remove</button>` : ""}</div>` : "";
  ...
  vendorForm.addEventListener("submit", async (event) => {
    ...
    try {
      const vendor = await window.BudgetAPI.addVendor({ name: vendorForm.elements.vendorName.value });
      vendorForm.reset();
      vendorForm.elements.vendorName.focus();
      formMessage.className = "vendor-form-message success";
      formMessage.textContent = window.BudgetAPI.getConfig().endpoint ? `${vendor.name} was added. Syncing…` : `${vendor.name} was added.`;
      render();
    } catch (error) {
  ...
  vendorList.addEventListener("click", (event) => {
    ...
    try {
      if (button.dataset.entityAction === "retry") window.BudgetAPI.retryEntity("vendor", button.dataset.entityId);
      if (button.dataset.entityAction === "remove" && window.confirm("Remove this unsynced vendor from this computer?")) {
  ...
  vendorList.addEventListener("keydown", (event) => {
    ...
    const row = event.target.closest("[data-entity-id]");
    if (!row) return;
    event.preventDefault(); window.EntityDetailUI?.open("vendor", row.dataset.entityId);
  });
