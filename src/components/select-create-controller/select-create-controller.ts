...
class SelectCreateController {
  ...
  #renderOptions() {
    ...
    const buttons = filtered.map((option) => {
      ...
      button.classList.add("fw-regular", "text-90");
      return button;
    });
  ...
  #exactMatch(value) {
    ...
    if (!key) return null;
    return (
      this.#options.find((option) => normalizeName(option.name) === key) || null
    );
  }
...
export const SelectCreateUtils = {
  cleanName,
  filterOptions,
  normalizeName,
  scoreOption,
};
export { SelectCreateController };
