// @ts-nocheck -- parsing inputs are intentionally polymorphic and validated at runtime.
  ...
  function headerSignature(headers) {
    return JSON.stringify(
      headers.map((header) => normalizeHeader(header.name ?? header)),
    );
  }
  ...
  function parseCSV(input) {
    ...
    if (quoted)
      throw new Error(`Unclosed quoted field beginning on line ${recordLine}.`);
    if (field !== "" || record.length || afterQuote) finishRecord();
    while (
      records.length &&
      records.at(-1).values.every((value) => value === "")
    )
      records.pop();
    ...
    const rows = records
      ...
      .map((entry) => {
        if (entry.values.length !== headers.length) {
          warnings.push(
            `Row ${entry.line} has ${entry.values.length} columns; ${headers.length} were expected.`,
          );
        }
        return {
  ...
  function columnIndex(mapping) {
    ...
    if (typeof mapping === "object")
      return Number.isInteger(mapping.index) ? mapping.index : null;
  ...
  function columnValues(parsed, mapping) {
    ...
    return parsed.rows
      .map((row) => String(row.values[index] ?? ""))
  ...
  function isNumericColumn(parsed, mapping) {
    ...
    return (
      values.length > 0 &&
      values.every((value) => Number.isFinite(parseNumber(value)))
    );
  }
  ...
  function suggestColumn(parsed, kind, predicate, allowFallback = true) {
    const candidates = parsed.headers
      ...
      .map((header) => ({
        index: header.index,
        score: headerScore(header, kind),
      }))
      .sort(
        (left, right) => right.score - left.score || left.index - right.index,
      );
    return candidates.length && (candidates[0].score > 0 || allowFallback)
      ? candidates[0].index
      : null;
  }
  ...
  function inferAmountSignConvention(parsed, mapping) {
    ...
    return {
      ...
      positive,
      convention:
        negative >= positive ? "expensesNegative" : "expensesPositive",
    };
  }

  function suggestBudgetMapping(parsed) {
    const numeric = (index) => isNumericColumn(parsed, index);
    const date = suggestColumn(
      parsed,
      "date",
      (index) => validDateFormats(parsed, index).length > 0,
    );
    ...
    const debitCredit =
      ...
      credit !== null &&
      debit !== credit &&
      headerScore(parsed.headers[debit], "debit") > 0 &&
      headerScore(parsed.headers[credit], "credit") > 0;
    const formats = validDateFormats(parsed, date);
    const dateFormat = formats.includes("MM/DD/YYYY")
      ? "MM/DD/YYYY"
    ...
    return {
      date,
      dateFormat,
      amountMode: debitCredit ? "debitCredit" : "unified",
      amount,
      debit: debitCredit ? debit : null,
      credit: debitCredit ? credit : null,
      ...
      personDescription: suggestColumn(parsed, "person", null, false),
      notes: suggestColumn(parsed, "notes", null, false),
    };
  }

  function suggestInvestmentMapping(parsed) {
    const numeric = (index) => isNumericColumn(parsed, index);
    const month = suggestColumn(
      parsed,
      "month",
      (index) => validMonthFormats(parsed, index).length > 0,
    );
    const balance = suggestColumn(
      parsed,
      "balance",
      (index) => index !== month && numeric(index),
      false,
    );
    const contributions = parsed.headers
      .filter(
        (header) =>
          header.index !== month &&
          header.index !== balance &&
          numeric(header.index) &&
          headerScore(header, "contribution") > 0,
      )
      .map((header) => header.index);
    const formats = validMonthFormats(parsed, month);
    const dateFormat = formats.includes("YYYY-MM")
      ? "YYYY-MM"
  ...
  function deriveBudgetAmount(row, type) {
    ...
    if (row.sourceDirection === "debit")
      return (
        Math.round(
          (type === "expense"
            ? Math.abs(row.sourceAmount)
            : -Math.abs(row.sourceAmount)) * 100,
        ) / 100
      );
  ...
  function fillBlankMatches(rows, field, key, keyForRow, value, apply) {
    ...
    const changed = rows.filter(
      (row) => !row.queued && !row[field] && keyForRow(row) === key,
    );
    changed.forEach((row) => {
      row[field] = value;
      apply?.(row);
  ...
  function createBudgetRows(
    ...
    const personMappings = new Map(
      (bundle.personMappings || [])
        .filter((item) => item.active !== false)
        .map((item) => [item.normalizedSourceDescription, item.assignmentId]),
    );
    const amountSignConvention =
      mapping.amountSignConvention ||
      (Number(profile.amountMultiplier) === -1
        ? "expensesNegative"
        : "expensesPositive");
    return parsed.rows.map((source) => {
      const vendorDescription = String(
        valueAt(source, mapping.vendorDescription),
      );
      const categoryDescription = String(
        valueAt(source, mapping.categoryDescription),
      );
      const personDescription = String(
        valueAt(source, mapping.personDescription),
      );
      const normalizedVendorDescription =
        normalizeDescription(vendorDescription);
      const normalizedCategoryDescription =
        normalizeDescription(categoryDescription);
      const normalizedPersonDescription =
        normalizeDescription(personDescription);
      let sourceAmount,
        sourceDirection = "unified",
        amountLayoutError = "";
      if (profile.amountMode === "debitCredit") {
        ...
        if (Number.isNaN(debit) || Number.isNaN(credit))
          amountLayoutError = "Debit and credit values must be numeric.";
        else if (hasDebit === hasCredit)
          amountLayoutError = hasDebit
      ...
      const base = {
        ...
        include: true,
        queued: false,
        originalValues: source.values.slice(),
        date: parseDate(valueAt(source, mapping.date), profile.dateFormat),
        amount,
        sourceAmount,
        sourceDirection,
        ...
        vendorResolution: vendorMappings.has(normalizedVendorDescription)
          ? "saved"
          : "unresolved",
        personId: normalizedPersonDescription
          ? personMappings.get(normalizedPersonDescription) || ""
          : references.sharedAssignmentId,
        personResolution: normalizedPersonDescription
          ? personMappings.has(normalizedPersonDescription)
            ? "saved"
            : "unresolved"
          : "default",
        categoryId: "",
        type: "",
        notes: String(valueAt(source, mapping.notes)),
        warnings: [],
        errors: [],
      };
      ...
      if (mapping.autoPopulateCategory && normalizedCategoryDescription) {
        const categoryMatches = references.categories.filter(
          (item) =>
            item.active !== false &&
            normalizeDescription(item.name) === normalizedCategoryDescription,
        );
        const category =
          categoryMatches.find((item) => item.type === inferredType) ||
      ...
      if (
        ...
        const vendor =
          entityNameMatch(references.vendors, vendorDescription) ||
          createDraftEntity?.("vendor", vendorDescription);
        if (vendor) {
          base.vendorId = vendor.id;
          base.vendorResolution = "pending";
        }
      }
      if (
        !base.personId &&
        mapping.autoPopulatePerson &&
        normalizedPersonDescription
      ) {
        const person =
          entityNameMatch(references.people, personDescription) ||
          createDraftEntity?.("assignment", personDescription);
        if (person) {
          base.personId = person.id;
          base.personResolution = "pending";
        }
      }
      return base;
    });
  ...
  function validateBudgetRow(row, references, profile) {
    ...
    const category = references.categories.find(
      (item) => item.id === row.categoryId && item.active !== false,
    );
    const assignment = references.people.find(
      (item) => item.id === row.personId && item.active !== false,
    );
    ...
    if (category?.type === "expense" && !vendor)
      errors.push("Choose a vendor for this expense.");
    if (!row.personDescription)
      warnings.push(
        "Shared assignment applied because no source person was available.",
      );
  ...
  function createInvestmentMonths(parsed, profile, existingMonths) {
    const mapping = profile.columnMapping || {};
    const contributionColumns = Array.isArray(mapping.contributions)
    ...
    parsed.rows.forEach((source, sourceIndex) => {
      const parsedDate = parseDate(
        valueAt(source, mapping.month),
        profile.dateFormat,
      );
      const month =
        parsedDate && parsedDate.length === 10
          ? parsedDate.slice(0, 7)
          : parsedDate;
      if (!months.has(month)) {
        const existing =
          existingMonths.find(
            (item) =>
              item.accountId === profile.investmentAccountId &&
              item.month === month,
          ) || null;
        months.set(month, {
      ...
      contributionColumns.forEach((column, columnIndexInMapping) => {
        ...
        record.flows.push({
          ...
          sourceDate:
            parsedDate || String(valueAt(source, mapping.month)).trim(),
          sourceColumn:
            parsed.headers[sourceColumnIndex]?.label ||
      ...
      if (
        balanceColumn !== null &&
        String(valueAt(source, balanceColumn)).trim() !== ""
      ) {
        const sortDate =
          parsedDate?.length === 10 ? parsedDate : `${month || ""}-00`;
        if (
          sortDate > record._balanceSortDate ||
          (sortDate === record._balanceSortDate &&
            sourceIndex >= record._balanceSourceIndex)
        ) {
          record.balance = parseNumber(valueAt(source, balanceColumn));
          record.balanceOrigin = "csv";
          record.balanceSourceDate =
            parsedDate || String(valueAt(source, mapping.month)).trim();
    ...
    return [...months.values()].map((record) => {
      ...
      record.warnings = record.existing
        ? ["This month already exists and will be replaced."]
        : [];
  ...
  function validateInvestmentMonth(row, references, profile) {
    const errors = row.errors.filter((error) =>
      error.includes("more than once"),
    );
    const warnings = row.existing
      ? ["This month already exists and will be replaced."]
      : [];
    if (!profile?.id) errors.push("Choose and save an import profile.");
    if (
      ...
      errors.push("Choose an active investment account.");
    if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(row.month || ""))
      errors.push("Enter a valid reporting month.");
    if (
      row.balance === null ||
      row.balance === "" ||
      ...
      errors.push("Enter a nonnegative ending balance.");
    if (
      row.flows.some(
        (flow) =>
          !Number.isFinite(Number(flow.amount)) || Number(flow.amount) === 0,
      )
    )
      errors.push("Cash-flow values must be non-zero numbers.");
  ...
  export const ImportUtils = {
    ...
    headerScore,
    suggestColumn,
    inferAmountSignConvention,
    suggestBudgetMapping,
    suggestInvestmentMapping,
    deriveBudgetAmount,
    suggestBudgetType,
    entityNameMatch,
    fillBlankMatches,
    createBudgetRows,
    validateBudgetRow,
    createInvestmentMonths,
    validateInvestmentMonth,
    createInvestmentRows,
    validateInvestmentRow,
  };
