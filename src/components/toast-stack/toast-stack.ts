export interface ToastOptions { type?: "error" | "success"; sticky?: boolean; duration?: number; action?: string; onAction?: () => void; }
export class ToastStack extends HTMLElement {
  ...
  show(text: string, options: ToastOptions = {}): HTMLElement {
    const toast = document.createElement("div"); toast.className = `toast${options.type ? ` ${options.type}` : ""}`;
    ...
    if (options.action) { const action = document.createElement("button"); action.type = "button"; action.className = "toast-action"; action.textContent = options.action; action.addEventListener("click", () => { options.onAction?.(); toast.remove(); }); actions.append(action); }
