...
export let OnboardingUI;
import templateString from "./template.html" with { type: "text" };
export class OnboardingOverlay extends HTMLElement { connectedCallback(): void { if (!this.dataset.initialized) { this.dataset.initialized = "true"; this.innerHTML = templateString; } } }
if (!customElements.get("onboarding-overlay")) customElements.define("onboarding-overlay", OnboardingOverlay);
...
(function () {
  ...
  function normalizeProgress() {
    ...
    const activeUser = APIs.budget.getActiveUser();
    if (endpoint)
  ...
  async function connectEndpoint(endpoint) {
    ...
    const previous = APIs.budget.getConfig();
    await APIs.budget.testConnection(value);
    APIs.budget.saveConfig({ endpoint: value });
    try {
      await APIs.budget.loadAppData({ refresh: true });
      ...
      APIs.budget.saveConfig(previous);
      throw new Error(
  ...
  function renderProfileForm() {
    ...
    setPrimary("Create profile", async () => {
      ...
      setMessage();
      try {
        const saved = await APIs.budget.addUser({ firstName, lastName });
        connectedUsers = [
  ...
  function renderProfilePicker() {
    ...
    setPrimary("Use this profile", () => {
      ...
      try {
        APIs.budget.setActiveUser(select.value);
        progress.step += 1;
        writeProgress();
        render();
      } catch (error) {
  ...
  async function renderProfileStep() {
    if (!connectedUsers) {
      ...
      try {
        connectedUsers = APIs.budget.listUsers();
        profileMode = connectedUsers.length ? "select" : "create";
      } catch (error) {
  ...
  function renderBackButton(stepKey) {
    const endpointSaved = Boolean(APIs.budget.getConfig().endpoint);
    const canGoBack =
