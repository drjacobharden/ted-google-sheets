...
const { shortMonthNames } = DateUtils;
...
class MonthPicker extends HTMLElement {
  ...
  get alignment() {
    ...
    return val === "right" || val === "center" ? val : "left";
  }
  ...
  connectedCallback() {
    const alignmentClass = `month-picker-popover-${this.alignment}-align`;
    ...
    this.addEventListener("popover-dismiss", this);
  ...
  disconnectedCallback() {
    ...
    this.removeEventListener("popover-dismiss", this);
  ...
  #handleClick(event) {
    if (event.currentTarget === document) {
      if (!this.contains(event.target)) this.#closeCalendar();
      return;
    }
  ...
  #openCalendar() {
    ...
    this.#popover.show(this.#trigger, {
      side: "bottom",
      align:
