...
export class BudgetOverviewScreen2
  extends HTMLElement
  implements EventListenerObject
{
  #periodControl!: HTMLElement;
  #periodInputs: HTMLInputElement[] = [];
  #chart!: HTMLElement;
  #metrics!: HTMLElement;
  ...
  connectedCallback(): void {
    ...
    this.#listening = true;
    this.#periodControl.addEventListener("change", this);
    ...
    void document.fonts.ready.then(() => {
      if (this.isConnected) this.#renderPeriodControl();
    });
  }
  ...
  disconnectedCallback(): void {
    ...
    this.#periodControl.removeEventListener("change", this);
  ...
  handleEvent(event: Event): void {
    ...
    if (
      event.type === "change" &&
      event.currentTarget === this.#periodControl &&
  ...
  #captureElements(): void {
    ...
    this.#periodControl = this.querySelector<HTMLElement>(
      "#spend-trend-period",
    )!;
    this.#periodInputs = Array.from(
      this.#periodControl.querySelectorAll<HTMLInputElement>(
        'input[name="spend-trend-period"]',
      ),
    );
  ...
  #renderPeriodControl(): void {
    this.#periodInputs.forEach((input) => {
      input.checked = input.value === this.#period;
    });
    const selected = this.#periodInputs.find((input) => input.checked);
    const label = selected?.closest<HTMLLabelElement>("label");
    if (!label) return;
    this.#periodControl.style.setProperty(
      "--period-indicator-left",
      `${label.offsetLeft}px`,
    );
    this.#periodControl.style.setProperty(
      "--period-indicator-width",
      `${label.offsetWidth}px`,
    );
  }
...
if (!customElements.get("budget-overview-screen-2")) {
  customElements.define("budget-overview-screen-2", BudgetOverviewScreen2);
}
